...
As this is a promo website, there is no requirement for a back-end architecture to handle dynamic data or user interactions. The website will be deployed as a static website and hosted on a CDN, such as Amazon S3 or Netlify.
...
As this is a static website, there is no back-end implementation required. However, we will use a JSON file to store information about films and categories, which will be accessed dynamically using Handlebars templates.
